...
def start_timeline(timeline):
    ...
    for command in timeline:
        if command[0] == "ADD":
            agenttype = command[2]
            folder = command[3]
            l = launcher(agenttype=str.lower(agenttype), Qtosendunicast=Qtosendunicast, Qtosendbroadcast=Qtosendbroadcast, QtoHardwareManager=hardware_manager.QtoHardwareManager, folder=folder, t_b=t_b)
...
def main():
    ...
    1. ["ADD", ID, agenttype, folder , hardwareID] : 
